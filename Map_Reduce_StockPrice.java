		...
		public static class StockReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
			...
			public void reduce(Text key, Iterable<IntWritable> values,Context context) throws IOException, InterruptedException {
				...
				for (IntWritable val : values) {
					...
					//store smallest two in minValue1 and minValue2
					if (val.get() < minValue1) {
						minValue1 = val.get();
					} else if (val.get() >= minValue1 && val.get() < minValue2 ) {
						minValue2 = val.get();
				}
				//avg = sum / count;
				context.write(key, new IntWritable(maxValue));
				//context.write(key, new IntWritable(minValue1));
				//context.write(key, new IntWritable(minValue2));
			}
		}
